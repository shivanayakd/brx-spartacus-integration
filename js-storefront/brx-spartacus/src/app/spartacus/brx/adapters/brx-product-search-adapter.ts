...
import { ConverterService, Product, ProductSearchAdapter, ProductSearchPage, PRODUCT_NORMALIZER, PRODUCT_SEARCH_PAGE_NORMALIZER, PRODUCT_SUGGESTION_NORMALIZER, SearchConfig, Suggestion } from '@spartacus/core';
...
export class BrxProductSearchAdapter implements ProductSearchAdapter  {
  ...
  search(
    ...
    if (searchConfig.type === 'suggestion') {
      return this.http
        .get(this.getSuggestionEndpoint(query))
        .pipe(
  ...
  loadSuggestions(
    ...
    return this.http
      .get(this.getSuggestionEndpoint(term))
      .pipe(
  ...
  protected getSuggestionEndpoint(
    term: string
  ): string {
